...
YouTubeSearchResult *youtube_parse_search(std::string url) {
	YouTubeSearchResult *res = new YouTubeSearchResult();
...
YouTubeSearchResult *youtube_parse_search(char *url) {
	std::string str(url);
	return youtube_parse_search(str);
}

YouTubeSearchResult *youtube_parse_search_word(char *search_word) {
	std::string str(search_word);

	std::string search_url = "https://m.youtube.com/results?search_query=";
	for (auto c : str) {
		search_url.push_back('%');
		search_url.push_back("0123456789ABCDEF"[(uint8_t)c / 16]);
		search_url.push_back("0123456789ABCDEF"[(uint8_t)c % 16]);
	}

	return youtube_parse_search(search_url);
}

YouTubeSearchResult *youtube_continue_search(const YouTubeSearchResult &prev_result) {
	YouTubeSearchResult *new_result = new YouTubeSearchResult(prev_result);
